...
- Homes Under Contract: ${r.current.underContract ?? "N/A"}
- Months of Inventory: ${r.current.moi != null ? r.current.moi.toFixed(1) : "N/A"}
- New Listings ${label}: ${r.current.newListings ?? "N/A"} (vs ${r.newListingsPrior ?? "N/A"} in ${priorLabel})
- Median Days on Market: ${r.current.medianDaysOnMarket ?? "N/A"} days
...
export async function generateQuarterlyRegionalReport(regions, {
  ...
  function tableRow(r, field, formatType, changeKey) {
    ...
    <tr>
      ...
      <td style="text-align:center;padding:7px 10px">${fmt(r.current[field], formatType)}</td>
      <td style="text-align:center;padding:7px 10px">${fmt(r.prior[field], formatType)}</td>
      ${changeCell(r.change[changeKey])}
    </tr>`;
  }
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* Print is tighter than screen: with the web fonts loaded a page renders taller than
       a sheet, which pushed the footer disclaimer onto a page of its own. Row padding is
       the biggest lever — a 25-row table saves ~1.5in. Cell padding is set inline, so
       these need !important. min-height leaves slack for the print dialog's own margins. */
    @media print {
      ...
      .analysis-body p, .analysis-body.dense p { font-size: 0.9em; line-height: 1.55; margin-bottom: 12px; }
